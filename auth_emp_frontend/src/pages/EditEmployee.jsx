...
const EditEmployee = () => {
  ...
  const designationOptions = ['HR', 'Manager', 'Sales', 'Software Developer', 'Web Developer'];
  ...
  const fetchEmployeeData = async () => {
    try {
      ...
      // Convert courses from string to array if needed
      const coursesArray = Array.isArray(employee.f_Course) 
        ? employee.f_Course 
        : employee.f_Course?.split(',').map(c => c.trim()) || [];
      ...
      setFormData({
        ...
        designation: employee.f_Designation || 'HR',
        gender: employee.f_gender || 'Male',
        course: coursesArray
      });
      
      // Set preview image if available
      if (employee.f_Image) {
        setPreviewImage(`http://localhost:5000/${employee.f_Image}`);
      }
      
      setLoading(false);
    } catch (error) {
      console.error('Error fetching employee data:', error);
      setLoadError('Failed to load employee data. Please try again later.');
      setLoading(false);
    }
  };
